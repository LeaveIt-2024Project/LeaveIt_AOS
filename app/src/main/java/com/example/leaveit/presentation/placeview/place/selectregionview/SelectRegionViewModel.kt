...
class SelectRegionViewModel @Inject constructor(
    private val getFeedUseCase: GetPlaceUseCaseInterface
) : ViewModel() {
    ...
    private val _placeData: MutableLiveData<PagingData<SelectRegionModel>> by lazy { MutableLiveData() }
    val placeData: LiveData<PagingData<SelectRegionModel>> = _placeData
    ...
    fun getAllPlaceData(regionStr: String) {
        viewModelScope.launch {
            getFeedUseCase.getAllPlaceUseCase(regionStr).cachedIn(viewModelScope).collectLatest { data ->
